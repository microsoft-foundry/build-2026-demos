...
* **Foundry Toolboxes are now in public preview.** Toolboxes are reusable, centrally managed bundles of tools (APIs, MCP servers, custom connectors) that any AI agent can consume through a single MCP-compatible endpoint. Build once, consume everywhere — no more duplicating tool integrations across agents.
...
* **Supported tool types at preview.** Web Search, Azure AI Search, File Search, Code Interpreter, custom MCP endpoints, OpenAPI services, and agent-to-agent (A2A) flows.
* **Framework-agnostic consumption.** Any agent that can call an MCP endpoint — whether built in Python, .NET, Java, or C# — can use a Toolbox.
* **Discover and Govern capabilities on the roadmap.** Search existing approved tools (Discover) and centralized analytics and controls (Govern) are coming. Build and Consume are live now.
